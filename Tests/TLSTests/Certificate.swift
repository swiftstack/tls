...
private let value: Certificates = .init(
    context: 0,
    sertificates: [.init(bytes: [0x42], extensions: [])])

private let record: Record = .init(
    version: .tls12,
    content: .handshake(.certificate(value)))

@Test
func `decode handshake certificates`() async throws {
    let result = try await Record.decode(from: bytes)
    #expect(result == record)
}

@Test
func `encode handshake certificates`() async throws {
