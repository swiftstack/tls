...
private let serverKeyExchange: ServerKeyExchange = .init(
    curve: .secp256r1,
...
@Test
func `decode server key exchange`() async throws {
    let result = try await ServerKeyExchange.decode(from: [UInt8](bytes[9...]))
    #expect(result == serverKeyExchange)
}

@Test
func `decode record -> handshake -> server key exchange`() async throws {
...
@Test
func `encode server key exchange`() async throws {
    let result = try await serverKeyExchange.encode()
    #expect(result[...] == bytes[9...])
}

@Test
func `encode record -> handshake -> server key exchange`() async throws {
    let record = Record(
        version: .tls12,
