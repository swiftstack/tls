...
private let clientKeyExchange: ClientKeyExchange = .init(
    pubkey: [UInt8](bytes[10...])
)

@Test
func `decode client key exchange`() async throws {
    let result = try await ClientKeyExchange.decode(from: [UInt8](bytes[9...]))
    #expect(result == clientKeyExchange)
}

@Test
func `decode record -> handshake -> client key exchange`() async throws {
...
@Test
func `encode client key exchange`() async throws {
    let result = try await clientKeyExchange.encode()
    #expect(result[...] == bytes[9...])
}

@Test
func `encode record -> handshake -> client key exchange`() async throws {
    let record = Record(
        version: .tls12,
